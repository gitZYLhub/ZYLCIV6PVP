...
- 修复：新增统一排除判定，地脉不再作为已有五产候选；升级分支原本已拒绝所有带资源地块，行为保持不变。保底日志新增候选资源索引。
- 本局复核：日志中的地脉索引为 `367326199`，此前 6 个结果坐标的资源索引均不是该值；但修复后从代码层面保证未来地图也不会把地脉算入五产。
- 验证：Windows PowerShell 5.1 与 PowerShell 7 的 `tools/validate.ps1` 均通过；universal、Windows、macOS 发布包均重建成功，且三类包内均包含该排除逻辑。
- 提交：待提交。
...
- 目标：把主键报告中的未解析计数扩展成完整、可审查的动态查询清单，先识别兼容性和顺序依赖，再决定后续精简候选。
- 范围：新增 `DatabaseInsertSelects.ps1`、`database-insert-select-contract.json` 和专项说明；分类接入数据库报告与总校验，并同步工程元数据、Manifest 说明、架构、计划、测试矩阵和工作日志。不修改运行 SQL/XML、动作图、ModInfo、玩法值、发布包、名称或版本号。
- 设计决定：分类器先以词法状态机遮蔽字符串、引号标识符和注释中的关键字，再提取真实 `FROM/JOIN` 源表；逐条固定路径、插入序号、目标/源表、Action/Criteria、冲突模式、语句哈希、连接/过滤/集合/分组/排序/嵌套/存在性和自读标记。分类器用含伪 `FROM/JOIN` 字符串、三种引号源表、连接、嵌套、集合查询、自读以及外层无 `FROM` 守卫的正反夹具自检。
- 结果：源码共有 381 条动态 `INSERT ... SELECT`，分布在 47 个文件、60 个目标表和 66 个直接源表；其中 371 条在主键分析中以 `insert-select` 为首要原因，另 10 条分别被 4 个项目自建无主键和 6 个官方无主键原因遮蔽。35 个 Action 中共有 208 条受 Criteria 门控；普通/Ignore/Replace 为 278/83/20。互斥结构为 simple 304、joined 33、nested 30、compound 8、grouped 6；另有 18 条仅存在性守卫和 58 条自读目标表，源表提取问题为 0。语义 SHA-256 为 `03ea2f82244fe69388bd17293da7725c2984145e8113c615ba320aeb046739fe`。
...
- 提交：`17525a7 fix: guard configuration string conversions`。

### 2026-09-09 / M4-配置数值使用缺值防护

- 目标：继续消除配置 getter 零返回值根因，覆盖不经过 `tonumber/tostring`、而是直接参加算术、大小比较或随机数入口的活跃路径。
- 范围：修改重开投票、前端选人过滤、等待室 `.rand` 命令和 BBG 传统征服检查四个运行脚本，扩展通用运行安全规则，并更新 CHANGELOG、架构、计划、测试矩阵与工作日志；不修改配置定义、正常种子算法、Ban/Pick 规则、胜利阈值、动作图、ModInfo、名称或版本号。明确休眠的 `NHK/UI/TurnTime_HotKey.lua` 不进入活跃扫描和发布包，本批不改。
- 盘点：新增门禁在 543 个活跃引用中命中 9 行：VotePanel 种子递增 2 行，SetupParameters 的 Ban 数值比较/受限选人种子 4 行，StagingRoom 随机命令 2 行，BBG 传统征服阈值除法 1 行。修复后活跃命中为 0；正例证明捕获、转换、兜底后再运算可通过且 Lua 注释不误报，反例覆盖 getter 位于算术符左右、大小比较和随机数入口。
- 实现：重开输入先解析一次；空输入时读取旧种子并以 `(tonumber(value) or 0) + 1` 递增，与 `hostgame.lua` 既有的缺值种子 `0` 约定一致。Ban 格式捕获一次并按配置声明的默认值 `0` 处理；受限选人和 `.rand` 的同步种子先转数字，缺值取 `0`。传统征服启用值和阈值各读取一次，阈值不可转数字时保留原有内部值 `1`（100%）。正常路径的实际配置值、随机入口参数、过滤分支和胜利计算不变。
...
- 提交：`0f54c25 fix: harden taoist runtime`。

### 2026-09-09 / M4+M9-道士事务加固与 2.0.0 收口

- 目标：按用户要求冻结扩展范围，完成当前高风险联机修复并把项目正式收口为 2.0.0。
- 保留行为：`Taoist_unit.sql` 的 `Units_MODE.ActionCharges = 1` 保持不变，地卜师出生自带 1 次，可立即创建 1 个地脉；Gameplay 单次创建仍消耗 1 次。该值新增独立数据契约及把 1 改为 0 的负向自测。
- 修复：四个道士操作先把玩家/坐标/单位 ID 转为数字，核对玩家、地块、单位类型及单位当前位置，再执行地脉修改或提示；创建还要求至少 1 次且目标无资源。缺少可选单位/地脉元数据时安全停用，不再空索引。
- 联机：占地刷新事务新增 `TaoistUnit`，UI 仅在事件属于本地玩家、事务单位等于充能变化单位且地块/城市仍有效时购地；Gameplay 恢复端再次核对单位、地块、城市和原金币属性，恢复后清空四项事务。过期坐标、其他单位和其他客户端不会重复产生购地/金币副作用。
- 幂等/性能：晋升补充只处理尚未写入对应 Promotion 属性的晋升，并把同次新增次数合并为一次 `ChangeActionCharges`；删除未使用的伤害事件函数、按钮城市局部量和原因返回值。
- 回归保护：道士运行时契约新增安全元数据、四操作上下文、修改前校验、本地客户端、单位事务和晋升幂等检查；内存反例覆盖远端客户端回调、过期坐标、重复晋升以及初始次数归零。
- 版本收口：`tools/project.json` 升为 `2.0.0`/ModInfo `200`，组装器同步 ModInfo、双语标题和 `ZYLPVPMOD v2.0.0` 多人握手；Database/Lua/Modding 四类证据契约同步身份和受保护哈希。新增迁移说明，Mod ID 保持不变。
- 验证：PowerShell 7 和 Windows PowerShell 5.1 均通过 203 XML、108 Criteria、280 Actions、1071 Files、543 活跃引用、48 休眠文件和 116 源码专用文件。最终构建：universal 1066 文件/776,215,205 字节/`7f5766c03b5e11e191570f6eccb8c09413adb679613312de3f9f19913f770605`；Windows 897/442,474,268/`a505731f32c90054173757045982ec7395c7efe2045c9199f61f356889abac28`；macOS 897/442,473,930/`851eef2e3406fe42bf8c79e391d064468d363d6858e8fbba8c475e2faede646e`，均在硬预算内。
- 边界：自动化已完成代码、静态和构建层收口；文明 VI 新局/读档、目标平台和双客户端联机仍需按测试矩阵人工验收。本项不再因此继续扩大代码重构范围。
- 提交：本次 2.0.0 收口提交。

### 2026-09-10 / M10-吸收 1.4.x 玩法变化

- 目标：以 1.3.0 为共同基准，识别 1.4.x 开发线的全部玩法差异，并将其适配到 2.0 模块化重构线。
- 对账：比较 `1852e1d..d9cddf7` 的 30 个变化文件及 `ZYLPVPMOD_1.4_大版本修改合集.md`。21 个运行时玩法文件全部映射到 2.0；`data/MP_helper.lua` 只含 1.4.2 握手版本差异，ModInfo/组装器/旧校验器属于 1.4 发布包装，均不覆盖 2.0。
- 修改：移植俄罗斯拉夫拉城市冻土信仰和开局加成主要玩家筛选；重建普通组队 Rich Mainland 的 FFA 基底与 2 列接缝；加入队内前后排、横向大陆、环形大陆及 1.4.2 的横向缩短、东西岛屿/浅海、环形碎片岛、接缝深海和横向条带大陆。横向地图英中说明同步修正为最终 1.4.2 行为。
- 重构适配：新增横向/环形配置、文本、入口脚本和 SQL，并登记到 `07-rich-mainland` 的 Criteria、FrontEnd、InGame 和 Files 分域片段；重新组装 2.0 ModInfo。数据库动作图、主键、Insert-Select 和写集合契约更新到四个按 `MAP_SCRIPT` 互斥的地图配置提供者；互斥组显式不要求单 profile 最终值探针。
- 保留：AssignStartingPlots、Rich Mainland 核心和开局加成脚本继续保留 2.0 的三处“先捕获配置值、再 `tonumber`”安全修复；横向地图说明另修正为 1.4.2 最终的东西岛屿/浅海规则。除这四处有意适配外，21 个移植运行文件按 CRLF/LF 归一后与 1.4.2 玩法源一致。版本保持 2.0.0/200。
- 契约：地图模块验证扩展到四张地图的文件/动作/条件闭包、11 档尺寸、默认设置、入口标志、生成/出生关键路径、本地化和 1.4.2 负向回归；俄罗斯、开局加成及数据库重复键验证同步更新。
- 文档：更新 README、SOURCES、CHANGELOG、验收清单、迁移说明、架构、测试矩阵与 Manifest 说明；新增 `PORT_1.4_TO_2.0.md` 逐项迁移台账。
