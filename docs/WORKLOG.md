...
- 修改：新增 `.gitattributes`、`.gitignore`、`CHANGELOG.md` 以及 `docs` 下的治理文档；校验器识别源码专用文件。
- 验证：`git diff --check` 无空白错误；`tools/validate.ps1` 通过，结果为 164 XML、108 条件、283 动作、1077 文件、549 活跃引用、48 个休眠文件和 9 个源码专用文件。
- 风险/待办：现有 ModInfo 和校验器仍为超大单文件，后续里程碑拆分。
- 提交：待提交。
