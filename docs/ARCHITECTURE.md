...
- `MultiplayerChecks.ps1` 承接大厅身份配置、事件生命周期、正式日志、周期读取、请求式完整刷新，以及赛事设置/启用 Mod 能力/玩家昵称三个失效缓存，另保护握手状态转换、批量广播、随机领袖、匿名昵称、投票重开、断线控制、重同步、突然死亡、主菜单和主回合计时器；领域调度器统一读取六个 UI 源并返回问题，独立自检函数对默认阶段全扫、三个缓存回退及其他联机路径构造十一类内存漂移。
