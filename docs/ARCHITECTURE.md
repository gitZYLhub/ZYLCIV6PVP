...
- `MultiplayerChecks.ps1` 承接大厅身份配置、事件生命周期、正式日志、周期读取、请求式完整刷新、握手状态转换、批量广播、随机领袖、匿名昵称，以及投票重开、断线控制、重同步、突然死亡、主菜单和主回合计时器契约；领域调度器统一读取六个 UI 源并返回问题，独立自检函数对洗牌、握手转换、默认阶段全量扫描、投票、掉线、重同步、突然死亡和主菜单注销构造八类内存漂移，不在主入口重复文件调度细节。
