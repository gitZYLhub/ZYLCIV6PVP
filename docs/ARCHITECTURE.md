...
- `DatabasePrimaryKeys.ps1` 在 Schema 覆盖层之上解析官方表及项目 `CREATE TABLE` 的表级/列级主键与列序，并保守解析 SQL `VALUES`（显式列及列序一致时的隐式列）和 XML 属性行；只有全部主键字段为字面量时才输出行候选。`INSERT ... SELECT`、缺少主键字段和真实无主键表分别记录未解析原因；跨文件同主键分组同时比较完整字面量行、Criteria、动作顺序和冲突模式，只有更早无条件同值写入支配更晚 `INSERT OR IGNORE` 时才给出可移除位置。
