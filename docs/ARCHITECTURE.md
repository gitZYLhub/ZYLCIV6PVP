...
- `UiChecks.ps1` 逐步承接 UI/QoL 的文件、Criteria/Action、上下文和安全源码契约；现覆盖首批 TPT 功能、Better Trade Screen 完整链，以及 Better Deal Window、Detailed Map Tacks、NHK 与外交条之间的唯一 Context、阻断 ID、可见性和兼容层，并以真实源码和错误动作反例自检。
