...
- `UiChecks.ps1` 逐步承接 UI/QoL 的文件、Criteria/Action、上下文和安全源码契约；现覆盖强制结束回合、LAN 名称长度、开局提示、禁地图标记、随机晋升快捷键、BBG UnitPanel 合并所有权，以及 Better Trade Screen 的完整加载链、排序、缓存和 BBG 收益兼容，并以真实源码和错误动作反例自检。
