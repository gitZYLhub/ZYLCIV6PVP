...
-- Map.GetMapSize() returns a database row index, so custom map-size rows cannot
-- safely be compared with the base game's fixed 0-5 indices.
function ZYL_RVC_GetMapRow()
	return GameInfo.Maps[Map.GetMapSize()];
end

function ZYL_RVC_GetNormalizedMapSize()
	local mapRow = ZYL_RVC_GetMapRow();
	local mapSizeType = mapRow and mapRow.MapSizeType or "MAPSIZE_STANDARD";
...
function IsAdjacentToRiver(iX, iY)
	...
	if (Map.GetPlot(iX, iY):IsRiver() == true) then
			return true;
	end
...
function TPT_AddGoodies(iW, iH, arg)
	local NO_PLAYER = -1;
	print("-------------------------------");
	...
	arg = arg or {}
	local TilesPerGoody = arg.TilesPerGoody or 60
	local GoodyRange = arg.GoodyRange or 5
	-- Check XML for any and all Improvements flagged as "Goody" and distribute them.
	local iImprovements = 0;
	local iTiles = 0;
	for improvement in GameInfo.Improvements() do
		...
		if(improvement.Goody and not (improvement.TilesPerGoody == nil)) then
			...
			if improvement.ImprovementType == "IMPROVEMENT_GOODY_HUT" then		-- 号码菌:固定村庄密度
				improvement.TilesPerGoody = TilesPerGoody
				improvement.GoodyRange = GoodyRange
			end
