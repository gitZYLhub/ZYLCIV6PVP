...
function BBS_GenerateTerrainTypes(plotTypes, iW, iH, iFlags, bNoCoastalMountains, temperature, notExpandCoasts, iDesertPercentArg, iPlainsPercentArg, fSnowLatitudeArg, fTundraLatitudeArg, fGrassLatitudeArg, fDesertBottomLatitudeArg, fDesertTopLatitudeArg)
	...
	local iPlainsPercent = 60; 
	local fSnowLatitude  = 1;-- + coldShift; -- was 0.86 put 0.84
	local fTundraLatitude = 0.65;-- + coldShift; -- was 0.65 put 0.67
	local fGrassLatitude = 0.1; 
	local fDesertBottomLatitude = 0.4; 
	local fDesertTopLatitude = 0.54; -- was 0.56 should be 0.6 to make the map symmetrical
	...
	local iDesertTopPercent		= 3;--100;
	local iDesertBottomPercent	= 3;--math.max(0, math.floor(100-iDesertPercent));
	...
	print("-"); print("BBS- Desert Percentage:", iDesertPercent);
