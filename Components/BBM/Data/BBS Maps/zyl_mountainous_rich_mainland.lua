------------------------------------------------------------------------------
--	FILE:	 zyl_mountainous_rich_mainland.lua
--	DERIVED: ZYL Lightweight Balance 的富饶竖向大陆
--	PURPOSE: 多山富饶大陆；保留竖向大陆、群岛、富饶和道路逻辑，
--            使用常规大陆板块盖章并温和提高山脉密度。
...
function BBS_Assign(args)
	...
	print("ZYLMRM: assigning starting plots")
...
function GenerateMap()
	...
	print("ZYLMRM: Mountainous Rich Mainland generation started")
	local pPlot;
	...
	-- 分配大陆
	print("ZYLMRM: stamping conventional continent plates");
	...
	local nwGen = NaturalWonderGenerator.Create({
		numberToPlace = math.max(0, math.floor((mapRow and mapRow.NumNaturalWonders or 4) + RichNum / 3)),
	});
	...
	AreaBuilder.Recalculate();
	TerrainBuilder.AnalyzeChokepoints();
	ZYLMRM_LogFinalStatistics();
	--[[ Corrupted legacy log text retained only as a comment.
	print("地图：富饶竖向大陆生成完毕");
	]]
	print("ZYLMRM: Mountainous Rich Mainland generation complete")
end
...
function ZYLMRM_LogFinalStatistics()
	local landCount = 0;
	...
	Game:SetProperty("ZYLMRM_LAND_TILES", landCount);
	Game:SetProperty("ZYLMRM_MOUNTAIN_TILES", mountainCount);
	Game:SetProperty("ZYLMRM_ISLAND_LAND_TILES", islandLandCount);
	Game:SetProperty("ZYLMRM_CONTINENT_MODE", "STAMPED");
	print(string.format("ZYLMRM mountains: %d / %d land tiles (%.2f%%)", mountainCount, landCount, mountainPercent));
	print("ZYLMRM offshore island land:", islandLandCount, "tiles");
end
...
function ZYL_RVC_EnforceSeaResourceRules()
	...
	for resourceIndex, removedCount in pairs(removedByType) do
...
function TeamPVPGenerateContinents(plotTypes)
	-- The source map rewrote continent IDs in row/column traversal order when
	-- directional team spawns were enabled.  That produced horizontal or
	-- vertical stripes unrelated to geography.  Team spawn direction now
	-- affects starts only; continent plates use Civ VI's ordinary stamp pass.
...
function AddFeatures()
	...
	local mapRow = ZYL_RVC_GetMapRow();
	local numLargeLakes = mapRow and mapRow.Continents or 1;
	-- 705：通过降雨调整大湖
