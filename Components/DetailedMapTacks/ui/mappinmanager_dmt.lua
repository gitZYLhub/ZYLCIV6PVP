...
function OnMapPinFlagRightClick(playerID:number, pinID:number)
    ...
    if m_IsShiftDown and playerID == Game.GetLocalPlayer() then
        ...
        if playerCfg == nil then return; end
...
function ToggleMapPinVisibility()
    ...
    if m_MapPinListBtn ~= nil and not m_MapPinListBtn:IsSelected() then
...
function DeleteMapPin()
    ...
    if m_MapPinFlags ~= nil and not m_MapPinFlags:IsHidden() then
...
function OnInputActionTriggered(actionId:number)
    -- Keep MPH's lobby-level no-pins rule authoritative. NHK handles only its
    -- chat-pin shortcut, so Add/Delete/Toggle have exactly one listener.
    if IsNoPinsEnabled() then
        return;
    end
