...
Team PVP Balanced mod 结社 3.93 的实际结社平衡已作为独立、仅在秘密结社模式开启时加载的整合层加入，包括镀金船厂、四结社数值、吸血鬼城堡重做和免费结社头衔；四结社三级在文艺复兴时代解锁，四级提前至工业时代。黄金黎明沿用“上古维序者 2.3”的地卜师单位及其地脉移动操作，在解锁对外贸易和首次建成炼金术会所时分别从首都获得，单位被捕获时像伟人一样撤回首都。结社建筑美术经独立 `.dep` 路由加载，不再把 `Buildings.artdef` 直接交给 `UpdateArt`。LightweightBalance 的统一村庄发现机制也已加入：调查任意部落村庄时都会以 `100000` 的基础概率分别尝试发现四个结社。其资源便利规则同样已加入：奢侈资源可移除为 40 金币，战略资源可移除为 20 生产力，地脉可移除为 40 金币或在其上修建农场。
...
马里不再承受所有城市 -5% 生产力，也不再于完成“对外贸易”后让首都或沙漠城市获得 +2 信仰；除市中心外，无地貌的沙漠和沙漠丘陵改为 +2 食物、+1 生产力、+1 信仰。曼丁哥市场的金币/信仰购买折扣由 20% 调回 10%，曼萨·穆萨的银行业贸易路线容量改回 Firaxis 原版规则：每次进入黄金时代时永久 +1 贸易路线容量。普通绿洲则作为全局地貌统一提升为 4 食物、1 金币，适用于所有文明。文明/领袖规则同时完成一次最终数据库反向审计，修正了马里、阿拉伯、中国、埃及、法国、格鲁吉亚、印度尼西亚、刚果、韩国、毛利、腓尼基、罗马、西班牙、苏美尔、瑞典等过时的简中文本，以及斯基泰中世纪战斗加成的错误预览文本 ID；还清理了西班牙传教团指向已删除旧产出 Modifier 的三条孤儿挂接。未把旧文本中已经被 BBG 删除的能力重新加入游戏。
