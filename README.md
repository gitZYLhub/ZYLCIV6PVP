...
Team PVP Balanced mod 结社 3.93 的实际结社平衡已作为独立、仅在秘密结社模式开启时加载的整合层加入，包括镀金船厂、四结社数值、吸血鬼城堡重做和免费结社头衔。结社建筑美术经独立 `.dep` 路由加载，不再把 `Buildings.artdef` 直接交给 `UpdateArt`。LightweightBalance 的统一村庄发现机制也已加入：调查任意部落村庄时都会以 `100000` 的基础概率分别尝试发现四个结社。其资源便利规则同样已加入：奢侈资源可移除为 40 金币，战略资源可移除为 20 生产力，地脉可移除为 40 金币或在其上修建农场。
