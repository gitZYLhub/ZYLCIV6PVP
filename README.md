...
1.2.1 起的规则覆盖层取消 BBG 对中世纪及以后科技统一增加 5% 基础成本的改动，并把落后当前游戏时代的科技折扣设为 25%；天文导航只以前置航海术解锁，并允许所有陆地单位上船。箭术、铸铜术、游戏与娱乐、历史记录、人文主义采用 LightweightBalance 的尤里卡/鼓舞，军事战术、海军传统、封建主义采用 Team PVP 的版本。每个相邻奢侈资源为商业中心及其特色替代区域提供 +1 金币相邻加成。

所有官方单位的过时字段恢复为 Firaxis 原版规则，不再使用 BBG 提前设置的强制过时节点；其中破城槌和攻城塔恢复为解锁“市政工程”后普通过时，装甲突袭骑兵恢复为没有强制过时节点。BBG Expanded 自增单位仍保留其自身规则。

俄罗斯恢复一项 Team PVP 风格能力：拥有圣地或拉夫拉的城市，其冻土和冻土丘陵均 +1 信仰。马格努斯左一移除 +20% 增长，把“开拓者不消耗人口”从右一移到左一；右一的工业区建筑生产力加成由 25% 提升到 40%。

马里的沙漠信仰使用与城市中心地块相同的地块产出作用域，避免区域 Modifier 与地块条件混用导致不结算；规则仍按 BBG 设计在完成“对外贸易”后生效：首都和建立在沙漠/沙漠丘陵上的城市 +2 信仰。文明/领袖规则同时完成一次最终数据库反向审计，修正了马里、阿拉伯、中国、埃及、法国、格鲁吉亚、印度尼西亚、刚果、韩国、毛利、腓尼基、罗马、西班牙、苏美尔、瑞典等过时的简中文本，以及斯基泰中世纪战斗加成的错误预览文本 ID；还清理了西班牙传教团指向已删除旧产出 Modifier 的三条孤儿挂接。未把旧文本中已经被 BBG 删除的能力重新加入游戏。
