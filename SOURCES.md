...
- 工具箱：大厅默认值改由最后加载的 `ZYL_LobbyDefaults.xml` 统一覆盖，避免原先在 MPH/BBG 参数创建前执行 `Update` 而命中零行；新建房间、恢复默认和把 MPH 预设切回“无”都会恢复 BCY 全城市+最大值、休闲（宽松）计时、FFA 外交栏情报、时代长度优化、无蛮族、公司、秘密结社和两项 TPT 计时开关。`休闲（宽松）` 按 `当前回合数 + 70 + 4C + 2U + Δ` 计算，其中 `C/U` 是真人玩家中的单人最高城市数/单位数，`Δ` 沿用休闲阶段修正（第1–9回合 −25 秒、第45回合起 +40 秒、第90回合起再 +20 秒），并包含房主的手动时间调整；`p+`/`p++` 每回合合计最多使用 6 次。
