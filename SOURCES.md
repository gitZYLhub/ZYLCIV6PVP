...
生成的统一 Mod ID 为 `4dd01931-9d44-4a8a-8e74-712cba0f0072`，ModInfo 版本为 `127`，用户可见版本为 `1.2.7`。
...
- BBG / Team PVP / LightweightBalance 规则覆盖：新增默认开启的大厅时代长度选项，开启时标准速度下远古固定为 54 回合、古典和中世纪固定为 50 回合（联机速度分别为 27、25、25 回合）；取消 BBG 中世纪以后科技统一 +5% 基础成本，落后时代科技改为 -25%；天文导航恢复航海术+占星术双前置并保留所有陆地单位上船解锁，同时按用户指定来源覆盖八项尤里卡/鼓舞。
...
- 高棉与克里规则覆盖：高棉文明的圣地与河流相邻时获得标准 +1 信仰相邻加成，旧领袖专属河流信仰 Modifier 保持移除以防重复；庞德梅克派出的贸易路线按目的地每座营地或牧场获得的食物由 +1 降至 +0.5，反向贸易路线的 +1 金币不变。
