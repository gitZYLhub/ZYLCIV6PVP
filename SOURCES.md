...
- 开局同步：`TurnProcessing.lua` 在互联网/局域网新游戏首个回合建立房主控制的载入屏障；每个真人参赛客户端仅在载入画面已经关闭且 `LoadGameViewStateDone` 已确认地图视图完成后，才通过 MPH 隐藏控制消息回报。全部完成前由房主保持网络暂停并在各端显示全屏输入拦截。断线者必须重连完成载入，或由房主正常处理其槽位；观察者和云端游戏不参与。`pausepanel.lua` 在屏障期间隐藏普通暂停解除入口。
