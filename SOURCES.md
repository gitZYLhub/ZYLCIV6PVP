...
生成的统一 Mod ID 为 `4dd01931-9d44-4a8a-8e74-712cba0f0072`，ModInfo 版本为 `200`，用户可见版本为 `2.0.0`。
...
- 工具箱：大厅默认值改由最后加载的 `ZYL_LobbyDefaults.xml` 统一覆盖，避免原先在 MPH/BBG 参数创建前执行 `Update` 而命中零行；新建房间、恢复默认和把 MPH 预设切回“无”都会恢复 BCY 全城市+最大值、休闲（宽松）计时、FFA 外交栏情报、时代长度优化、无蛮族、俘获移民送回家、公司、秘密结社和两项 TPT 计时开关。各地图尺寸默认城邦数在原设置上增加 2。`休闲（宽松）` 按 `当前回合数 + 70 + 4C + 2U + Δ` 计算，其中 `C/U` 是真人玩家中的单人最高城市数/单位数，`Δ` 沿用休闲阶段修正（第1–9回合 −25 秒、第45回合起 +40 秒、第90回合起再 +20 秒），并包含房主的手动时间调整；`p+`/`p++` 每回合合计最多使用 6 次。
- 工具箱：新增默认关闭的“开局加成玩家/开局加成”大厅设置，按真人参赛者在大厅中从上到下的次序选择 1 至 12 号玩家，并可一次性发放建造者、侦察兵或两者；空位、AI 和观察者不计入编号，发放状态随存档保存以防读档重复。
- 工具箱：新增默认关闭的纯大厅“三国杀”身份发牌。房主按真人大厅顺序配置主公、忠臣、反贼、内奸以及两名必须分属不同阵营的玩家；主忠与反贼分别成营，每名内奸独立成营。大厅负责随机和同步遮挡身份卡，真人名单或设置变化会使旧牌失效；游戏内只保留读取大厅结果的身份查看 UI，不注册身份 Gameplay 脚本，也不修改队伍或外交。
...
- BBG / Team PVP / LightweightBalance 规则覆盖：黑暗/普通和黄金时代基础分界分别设为 20/25；新增默认开启的大厅时代长度选项，开启时标准速度下远古至信息时代固定为 50/46/46/42/42/40/40/40 回合（联机速度分别为 25/23/23/21/21/20/20/20/20 回合）；取消 BBG 中世纪以后科技统一 +5% 基础成本，落后时代科技改为 -25%；天文导航改为研究航海术后即可直接研究，并保留所有陆地单位上船解锁，同时按用户指定来源覆盖八项尤里卡/鼓舞。
- 地图与文明规则覆盖：四张富饶大陆均使用共享生成、资源和出生平衡核心；重建后的组队PVP富饶大陆与 FFA 对齐 2～12 人尺寸和内容画布，只改变队伍半区、横向条带大陆编号和可选队内前后排；FFA/组队纵向图的 WrapX 深海接缝缩为 2 列。新增横向大陆和环形大陆，并吸收 1.4.2 的横向缩短、东西岛屿/浅海、环形零散内海岛屿、边界深海和横向条带大陆细化。开局加成按有文明的存活主要玩家编号，真人与 AI 均可成为目标，观察者排除。奢侈资源为商业中心及特色替代区域提供 +1 金币相邻；俄罗斯建立拉夫拉后，其城市内所有冻土和冻土丘陵 +1 信仰（包括市中心且不要求相邻）；法国文明全体领袖拥有覆盖全部已启用奢侈资源的 T4 出生关联，寻欢作乐凯瑟琳的已改良奢侈/加成/战略资源 +1 文化分别在技艺/封建主义/城堡解锁；毛利水运单位+2移动力改为“航海术”解锁，未改良树林与雨林首段+1生产力改为“对外贸易”解锁；大哥伦比亚恢复所有单位开局 +1 移动力；图拉真的免费市中心建筑提前至对外贸易；高卢在铸铜术后所有矿山 +1 文化；马里移除城市生产力惩罚及对外贸易城市信仰，非市中心无地貌沙漠/沙漠丘陵改为 +2 食物、+1 生产力，沙漠/沙漠丘陵市中心 +2 信仰，所有矿山 -1 生产力、+4 金币；曼丁哥市场购买折扣调回 10%，曼萨·穆萨恢复每次进入黄金时代永久 +1 贸易路线容量；普通绿洲全局调整为 4 食物、1 金币；马格努斯左一移除增长并接收免人口移民，右一工业区建筑加成改为 40%。
- 高棉与克里规则覆盖：高棉文明的圣地与河流相邻时获得标准 +1 信仰相邻加成，旧领袖专属河流信仰 Modifier 保持移除以防重复；庞德梅克派出的贸易路线按目的地每座营地或牧场获得 +1 食物，反向贸易路线的 +1 金币不变。原生目标城市改良统计效果没有数量上限参数，不使用无法限制内部改良计数的 `SubjectStackLimit` 冒充封顶。
- 秘密结社覆盖：血色契约 1 级的军营建造加速由 +50% 改为军营及其中建筑均 +15%；吸血鬼基础移动力恢复为 2，血色契约 2 级新增仅作用于吸血鬼的 +1 移动力，因此晋升前后总移动力分别为 2/3。
