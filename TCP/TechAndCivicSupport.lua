...
cached_turn = -1;
cached_extra_techboost = 0;
cached_extra_civicboost = 0;
...
function RealizeTurnsLeft( kControl:table, kData:table)
	...
	if kData ~= nil then
		kControl.TurnsLeft:SetToolTipString("");		-- 号码菌
		if isLastCompleted then										-- 刚完成
			kControl.TurnsLeft:SetToolTipString("");
		elseif kData.Boosted or not kData.Boostable then										-- 已经触发提升或者不可提升
			kControl.TurnsLeft:SetToolTipString(Locale.Lookup("LOC_TCP_PROGRESS_TOOLTIP",GetNumfone(kData.Progress*kData.Cost),kData.Cost));
		else
			kControl.TurnsLeft:SetToolTipString(Locale.Lookup("LOC_TCP_PROGRESS_TOOLTIP",GetNumfone(kData.Progress*kData.Cost),kData.Cost)..Locale.Lookup("LOC_TCP_ESTIMATES_TOOLTIP",GetNumfone(kData.Estimates),kData.Cost));
		end
		if kData.Enough then
			kControl.TurnsLeft:SetToolTipString(Locale.Lookup("LOC_TCP_PROGRESS_TOOLTIP",GetNumfone(kData.Progress*kData.Cost),kData.Cost)..Locale.Lookup("LOC_TCP_PROGRESS_ENOUGH_TOOLTIP"));
		end
	end
...
function GetResearchData( localPlayer:number, pPlayerTechs:table, kTech:table )
	...
	local boosted = Players[Game.GetLocalPlayer()]:GetTechs():HasBoostBeenTriggered(iTech)	-- 是否已经触发尤里卡？

	for row in GameInfo.Boosts() do
		if row.TechnologyType == techType then
			isBoostable	= true;					
			boostAmount = ((row.Boost + GetExtraBoostFromModifiers(Game.GetLocalPlayer(), true)) *.01 ) * researchCost;		--Convert the boost value to decimal and determine the actual boost amount.
	...
	local kData :table = {
		...
		Estimates		= pPlayerTechs:GetResearchProgress(iTech),		--预估值
		Enough			= false
	};
	if not kData.Boosted then		-- 未触发尤里卡
...
function RealizeCurrentResearch( playerID:number, kData:table, kControl:table )
	...
	if kData ~= nil then
		...
		kControl.TitleButton:SetText(Locale.ToUpper(kData.Name));		-- 号码菌 -- 判断能否通过尤里卡解锁科技，增加符号提示

		if kData.Enough then
			kControl.TitleButton:SetText(Locale.ToUpper(kData.Name)..Icon_Enough);
		end
...
function GetCivicData( localPlayer:number, pPlayerCulture:table, kCivic:table )
	...
	local boosted = Players[Game.GetLocalPlayer()]:GetCulture():HasBoostBeenTriggered(iCivic)	-- 是否已经触发尤里卡？

	for row in GameInfo.Boosts() do
		if row.CivicType == civicType then
			isBoostable	= true;					
			boostAmount = ((row.Boost + GetExtraBoostFromModifiers(Game.GetLocalPlayer(), false)) *.01 ) * progressCost;		--Convert the boost value to decimal and determine the actual boost amount.
			triggerDesc = row.TriggerDescription;
			break;
		end
	end
	...
	local kData :table = {
		...
		Estimates		= pPlayerCulture:GetTurnsToProgressCivic(iCivic),
		Enough			= false
	};
	if not kData.Boosted then		-- 未触发尤里卡
...
function RealizeCurrentCivic( playerID:number, kData:table, kControl:table, cachedModifiers:table )
	...
	if kData ~= nil then
		...
		kControl.TitleButton:SetText( Locale.ToUpper(kData.Name) );		--号码菌
