...
function PopulateNode(uiNode, playerTechData)
	...
	if live.Enough then
		uiNode.NodeName:SetText( Locale.ToUpper( techName )..Icon_Enough)	--号码菌
	end

	if live.Turns > 0 then
		if artInfo.Name == "BLOCKED" then		-- 号码菌 -- 获取正确回合数
			live.Turns = math.floor(math.max((live.Cost - live.Progress)/Players[Game.GetLocalPlayer()]:GetCulture():GetCultureYield(),1)+0.5)
		end		
...
function GetCurrentData( ePlayer:number )
	...
	for type,item in pairs(g_kItemDefaults) do
		...
		data[DATA_FIELD_LIVEDATA][type] = {
			...
			Enough		= false,
			Estimates   = playerCulture:GetCulturalProgress(civicID)
		}
		...
		if	(table.count(debugExplicitList) == 0 and debugFilterEraMaxIndex ==-1 ) or
			...
			for boostRow in GameInfo.Boosts() do
				if boostRow.CivicType == kEntry.Type then
					...
					kEntry.BoostAmount = boostRow.Boost + GetExtraBoostFromModifiers(Game.GetLocalPlayer(), false);	--号码菌--修正
					break;
				end
			end
...
function OnOpen()
	...
	g_kItemDefaults = PopulateItemData("Civics","CivicType","CivicPrereqs","Civic","PrereqCivic");	--号码菌-打开面板时刷新
