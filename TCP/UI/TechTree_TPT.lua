...
-- ===========================================================================
--[[
local function printTable(t, indent)		--号码菌 测试函数
    indent = indent or 0
...
function PopulateNode(uiNode, playerTechData)
	...
	if live.Enough then
		uiNode.NodeName:SetText( Locale.ToUpper( nodeName )..Icon_Enough);		--号码菌
	end
	

	if live.Turns > 0 then
		if artInfo.Name == "BLOCKED" then		-- 号码菌 -- 获取正确回合数
			live.Turns = math.floor(math.max((live.Cost - live.Progress)/Players[Game.GetLocalPlayer()]:GetTechs():GetScienceYield(),1)+0.5)
		end	
	...
	if item.IsBoostable and status ~= ITEM_STATUS.RESEARCHED and status ~= ITEM_STATUS.UNREVEALED then
		...
		if live.IsBoosted then
			...
			local boostAmount = (live.Estimates/live.Cost);		--号码菌
...
function GetCurrentData( ePlayer:number, eCompletedTech:number )
	...
	for type,item in pairs(g_kItemDefaults) do
		...
		data[DATA_FIELD_LIVEDATA][type] = {
			...
			Enough		= false,
			Estimates   = playerTechs:GetResearchProgress(techID)
		}
		if not data[DATA_FIELD_LIVEDATA][type].IsBoosted then
			--print(type.."没触发尤里卡,开始修正")
			if data[DATA_FIELD_LIVEDATA][type].BoostAmount then
				data[DATA_FIELD_LIVEDATA][type].Estimates = math.min(data[DATA_FIELD_LIVEDATA][type].Progress + math.floor(math.max(data[DATA_FIELD_LIVEDATA][type].Cost*data[DATA_FIELD_LIVEDATA][type].BoostAmount/100 -((data[DATA_FIELD_LIVEDATA][type].Cost*data[DATA_FIELD_LIVEDATA][type].BoostAmount/100 % 0.5 == 0) and 0.5 or 1),0)),data[DATA_FIELD_LIVEDATA][type].Cost)
			end
		end
		if data[DATA_FIELD_LIVEDATA][type].Estimates == data[DATA_FIELD_LIVEDATA][type].Cost then
			data[DATA_FIELD_LIVEDATA][type].Enough = true
		end
	...
	--print("----------------------------------输出表格GetCurrentData----------------------------------")		--号码菌--测试
	--printTable(data)
		...
		if	((table.count(debugExplicitList) == 0 and debugFilterTechMaxIndex ==-1 ) or
			...
			for boostRow in GameInfo.Boosts() do
				if boostRow.TechnologyType == kEntry.Type then
					...
					kEntry.BoostAmount = boostRow.Boost + GetExtraBoostFromModifiers(Game.GetLocalPlayer(), true);	--号码菌--修正
					break;
				end
			end
...
function OnOpen()
	...
	g_kItemDefaults = PopulateItemData();		--号码菌 打开面板时刷新
