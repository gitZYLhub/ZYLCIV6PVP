...
NPQC_BASE_OnShutdown = OnShutdown;
...
function NPQC_CreateClearButton()
	...
	local toolTip = Locale.Lookup("LOC_HUD_MAP_SEARCH_CLEAR_SEARCH");
...
function NPQC_ClearNotifications(playerID)
	if NPQC_notifications[playerID] == nil then return end
	...
	if NPQC_ClearButton ~= nil then NPQC_ClearButton.Top:SetHide(true); end
end	

function OnTPT_Settings_Toggle_NOC(ParameterId, Value)
	if ParameterId == "NotificationPanel_QuickClear" then
		HideClearButton = Value
		if NPQC_ClearButton ~= nil then NPQC_ClearButton.Top:SetHide(Value); end
		return
	end
end

function OnShutdown_NOC()
	LuaEvents.TPT_Settings_Toggle.Remove(OnTPT_Settings_Toggle_NOC)
	if NPQC_BASE_OnShutdown ~= nil then NPQC_BASE_OnShutdown() end
end

LuaEvents.TPT_Settings_Toggle.Add(OnTPT_Settings_Toggle_NOC)
OnShutdown = OnShutdown_NOC
