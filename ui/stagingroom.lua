...
-- Mod Flags
--------------------------------------------
local ZYLPVP_MOD_ID = "4dd01931-9d44-4a8a-8e74-712cba0f0072";
local ZYLPVP_BBG_VERSION = "7.4.6";
local ZYLPVP_BBM_VERSION = "1.39.1";
...
function GetLocalBBGVersion()
	return ZYLPVP_BBG_VERSION
end

function GetLocalBBMVersion()
	return ZYLPVP_BBM_VERSION
end
			...
			for i, player in ipairs(g_player_status) do
				...
				if player.Status == 2 then
					-- We haven't received an answer most likely hasn't a fully loaded MPH
						...
						if b_bbs_game == true and tostring(player.bbs_v) ~= tostring(GetLocalBBMVersion()) then
							Network.SendChat("[COLOR_Civ6Red]BBM Host version: "..tostring(GetLocalBBMVersion()).." Your version: "..tostring(player.bbs_v),-2,player.ID)
							player.Status = 66
						end
						if b_bbg_game == true and tostring(player.bbg_v) ~= tostring(GetLocalBBGVersion()) then
							Network.SendChat("[COLOR_Civ6Red]BBG Host version: "..tostring(GetLocalBBGVersion()).." Your version: "..tostring(player.bbg_v),-2,player.ID)
							player.Status = 66
						end
						if player.Status == 66 then
...
function SendVersion()
	...
	if localID ~= hostID and b_mph_game == true then
		local bbs_version = GetLocalBBMVersion()
		local bbg_version = GetLocalBBGVersion()
...
local function CanUseZYLHostControls()
	return Network.IsGameHost()
		...
		and not IsLaunchCountdownActive()
end

function RefreshZYLHostControls()
...
function OnZYLRandomTeams()
	if not CanUseZYLHostControls() then return end
	local participants = {}
	...
	if #participants < 2 then return end
...
function OnZYLToggleEmptySlots()
	if not CanUseZYLHostControls() then return end
	local openSlots = {}
	local firstClosed = -1
	for playerID = 0, g_currentMaxPlayers - 1 do
		local config = PlayerConfigurations[playerID]
		if config ~= nil then
			local status = config:GetSlotStatus()
			if status == SlotStatus.SS_OPEN then
				table.insert(openSlots, playerID)
			elseif status == SlotStatus.SS_CLOSED and firstClosed == -1 and not config:IsLocked() then
				firstClosed = playerID
			end
		end
	end
	if #openSlots > 0 then
...
function BuildAdditionalContent()
	...
	for _, curMod in ipairs(enabledMods) do
		...
		if curMod.Id == ZYLPVP_MOD_ID then
			modTitleStr =  "[COLOR_LIGHTBLUE]".. modTitleStr .. "[ENDCOLOR] (suite: "..tostring(GetLocalModVersion(curMod.Id)).."; BBG: "..ZYLPVP_BBG_VERSION.."; BBM: "..ZYLPVP_BBM_VERSION..")";
			b_mph_game = true
			b_bbg_game = true
			b_bbs_game = true
			s_bbg_id = curMod.Id
...
function Initialize()
	...
	g_version = GetLocalModVersion(ZYLPVP_MOD_ID)
