...
local g_showGreatPersonEra		:boolean= true;
...
function UnitFlag.UpdatePromotions( self )
	...
	-- Reset the presentation first because a recycled flag instance may have
	-- previously belonged to a great person.
	...
	local pUnit : table = self:GetUnit();
	if pUnit ~= nil then
		local unitRow = GameInfo.Units[pUnit:GetUnitType()]
		local unitType = unitRow and unitRow.UnitType or nil
		if unitType == "UNIT_BUILDER" or unitType == "UNIT_MILITARY_ENGINEER" then
			local buildCharges = pUnit:GetBuildCharges()
			if buildCharges ~= nil and buildCharges > 0 then
		...
		if g_showGreatPersonEra and (unitType == "UNIT_GREAT_GENERAL" or unitType == "UNIT_GREAT_ADMIRAL") then
			local greatPerson = pUnit:GetGreatPerson()
			local individual = greatPerson and greatPerson:GetIndividual() or -1
			local individualRow = individual >= 0 and GameInfo.GreatPersonIndividuals[individual] or nil
			local eraRow = individualRow and GameInfo.Eras[individualRow.EraType] or nil
			if eraRow ~= nil then
				self.m_Instance.UnitNumPromotions:SetText("[Size_14]" .. Locale.Lookup(eraRow.Name))
...
function OnUnitChargesChanged(playerID, unitID)
	...
	if flag ~= nil then flag:UpdatePromotions() end
end

function OnZYLSettingChanged(parameterId, value)
	if parameterId ~= "GreatGeneralEraReminder_Show" then return end
	g_showGreatPersonEra = value == true or value == 1
	Refresh()
end
...
function Unsubscribe()
	...
	LuaEvents.TPT_Settings_Toggle.Remove(OnZYLSettingChanged)
...
function Subscribe()
	...
	LuaEvents.TPT_Settings_Toggle.Add(OnZYLSettingChanged)
