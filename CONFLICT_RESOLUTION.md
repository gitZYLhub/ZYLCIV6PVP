...
- BBM 常规图、四张富饶大陆及其 Firaxis 回退路径都按领袖 ID 过滤海岸；同一优先级将来增加其他地形关联时也不会误删。
- 四张富饶大陆只对最终数据库中拥有海岸出生关联的陆地出生文明启用专用海岸分配。FFA 使用东西岸配额；普通组队与横向组队图服从玩家所属半区并优先本队外侧海岸；环形图按队内大厅顺序选择最北/最南外海岸或内海岸。BBG 已将毛利改为 `OceanStart=0` 并赋予 T1 海岸关联，所以毛利会参与；内陆版和最终仍直接出生在海洋中的领袖不进入这项陆地分配。
...
- `sql/ZYL_GameplayOverrides.sql` 在全部 BBG/BBM 数据库动作之后统一拥有科技时代折扣、天文导航的航海术单一前置及上船解锁、八项尤里卡/鼓舞、商业中心奢侈相邻和俄罗斯拉夫拉城市冻土信仰规则。
