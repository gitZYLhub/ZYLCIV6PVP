...
- 两张富饶大陆只对最终数据库中拥有海岸出生关联的陆地出生文明启用“东西岸优先、南北岸后备”。BBG 已将毛利改为 `OceanStart=0` 并赋予 T1 海岸关联，所以毛利会参与；内陆版和最终仍直接出生在海洋中的领袖不进入这项陆地分配。
...
- `sql/ZYL_GameplayOverrides.sql` 在全部 BBG/BBM 数据库动作之后统一拥有科技时代折扣、天文导航的航海术单一前置及上船解锁、八项尤里卡/鼓舞、商业中心奢侈相邻和俄罗斯冻土邻接圣地/拉夫拉信仰规则。
